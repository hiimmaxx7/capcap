# PROGRESS.md — Project Oculus

Nhật ký phát triển. Đọc file này trước khi đổi hành vi quay/UI để biết quyết định nào đã chốt và vì sao.

## Kiến trúc hiện tại

- .NET 8 WinForms, chạy dưới dạng tray icon (`TrayApplicationContext`), không cửa sổ chính.
...
- Âm thanh hiệu ứng (click/phím/cuộn) không phải thu mic — chỉ log timestamp qua low-level hook (`InputSoundLogger`) rồi tổng hợp sóng sin nhân tạo (`WavBuilder`), mux vào cuối bằng ffmpeg pass thứ 2.
...
- **Popup Lưu/Xóa sau khi dừng quay** — luôn hỏi trước khi giữ file lại trong `Videos\ProjectOculus`.
